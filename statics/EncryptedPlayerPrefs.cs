...
public static class EncryptedPlayerPrefs
{
    ...
    public static void SaveEncryption(string key, string type, string value)
    {
        ...
        string check = Md5(string.Concat(key, "_", type, "_", privateKey, "_", secretKey, "_", value));
        PlayerPrefs.SetString(string.Concat(key, "_encryption_check"), check);
        PlayerPrefs.SetInt(string.Concat(key, "_used_key"), keyIndex);
    }

    public static bool CheckEncryption(string key, string type, string value)
    {
        int keyIndex = PlayerPrefs.GetInt(string.Concat(key, "_used_key"));
        string secretKey = keys[keyIndex];
        string check = Md5(string.Concat(key, "_", type, "_", privateKey, "_", secretKey, "_", value));
        if (!PlayerPrefs.HasKey(string.Concat(key, "_encryption_check"))) return false;
        string storedCheck = PlayerPrefs.GetString(string.Concat(key, "_encryption_check"));
    ...
    public static void DeleteKey(string key)
    {
        ...
        PlayerPrefs.DeleteKey(string.Concat(key, "_encryption_check"));
        PlayerPrefs.DeleteKey(string.Concat(key, "_used_key"));
    }
